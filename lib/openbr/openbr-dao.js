...
var identifyUser = function (user) {
    ...
    fs.writeFile('./lib/openbr/temp/' + fileName, base64Data, 'base64', function (error) {
        if (error) {
            deferred.reject({message:error});
        } else {
            handler.galCompare(fileName, function(matches){
...
var enrollUser = function (user) {
    ...
    if(user.name && user.name.length ){
        ...
        deferred.reject({code: 403, message: "Empty name not allowed"});
    }
    return deferred.promise;
};
